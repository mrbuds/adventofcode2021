...
local function all_routes_from(a,finish, visited, joker_visit)
  if a == finish then return 1 end
  local local_visited = {}
  for k in pairs(visited) do local_visited[k] = true end
  if nodes[a].smallcave then
    if local_visited[a] then
      if joker_visit == false and a ~= "start" and a ~= "end" then
        joker_visit = true
      else
        return 0
      end
  ...
  for name in pairs(nodes[a].children) do
    count = count + all_routes_from(name, finish, local_visited, joker_visit)
  end
  return count
end

print(all_routes_from("start","end",{},false))